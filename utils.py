    ...
    if verbose:
        ...
        try:
            ...
            if time_difference >= 1:
                ...
                num_milliseconds = int(round(time_difference * 1000))
                print(f'{message if message is not None else "Command"} '
                      f'{"aborted after" if aborted else "took"} '
                      f'{num_milliseconds} '
                      f'{plural("millisecond", num_milliseconds)}')
    else:
        yield  # no-op
...
def print_df(df, all_rows=True, all_columns=True):
    """
    Prints the entirety of a polars DataFrame without truncating.
    
    Args:
        df: the DataFrame to print
        all_rows: whether to print all rows instead of just the default number
        all_columns: whether to print all columns instead of just the default
                     number
    """
    options = {}
    if all_rows:
...
def run_slurm(cmd, *, job_name='job', log_file=None,
    ...
    try:
        with NamedTemporaryFile('w', dir=os.environ.get('SCRATCH', 'Resources'),
...
def sparse_matrix_vector_op(sparse_matrix, op, vector, *, axis, inplace=False):
    ...
    if len(vector) != sparse_matrix.shape[axis]:
        ...
        raise ValueError(error_message)
    # Get the result's data type (note: // always returns an integer dtype)
    result_dtype = np.result_type(sparse_matrix.dtype, vector.dtype)
    if op == '//' and np.issubdtype(result_dtype, np.floating):
    ...
    if inplace:
        ...
        # `sparse_matrix`'s
        if inplace and result_dtype != data.dtype:
            error_message = (
                f'`sparse_matrix` has data type `{data.dtype}`, but '
                f'`sparse_matrix {op} vector` would have data type '
                f'`{result_dtype}`, so `inplace=True` cannot be specified')
            raise TypeError(error_message)
                ...
                def run_op_inplace(
                    ...
                    for j in range(indptr.shape[0] - 1):
                        for i in range(indptr[j], indptr[j + 1]):
                            data[i] {op}= vector[j]
            ''')['run_op_inplace'](data, indices, indptr, vector)
        else:
    ...
    # If not in-place...
        ...
        result = np.empty_like(sparse_matrix.data, dtype=result_dtype)
        # Get the Cython type of the result
        result_cython_type = cython_type(result_dtype)
        # Run the operation with Cython
        if isinstance(sparse_matrix, (csc_array, csc_matrix)) == axis:
            # Rowwise for CSR, or columnwise for CSC
            cython_inline(rf'''
                ...
                ''')['run_op'](data, indices, indptr, vector, result)
        else:
            # Rowwise for CSC, or columnwise for CSR
            cython_inline(rf'''
                def run_op(
...
def merge_pfiles(pfiles, merged_pfile, temp_dir=os.environ.get('SCRATCH', 'Resources'),
...
def munge_sumstats(raw_sumstats_files, munged_sumstats_file, REF, ALT, P, *,
    ...
    default_read_csv_kwargs = dict(na_values='NA')
...
def get_positions_bim_or_pvar(bim_or_pvar_file, new_bim_or_pvar_file, dbSNP, *,
    ...
    from dbSNP, saving the new file to new_bim_or_pvar_file.
    Variants not in dbSNP will have their chromosomes and positions set to -1.
    
    Use this function to remap bim/pvar files to a different genome build!
    ...
    # github.com/pola-rs/polars/issues/9773
    variants = get_positions(variants, dbSNP=dbSNP)\
        .with_columns(pl.col.CHROM.cast(str).str.slice(3).fill_null('-1'),
                      pl.col.BP.fill_null(-1))\
        .drop('FLIP')
    if verbose:
        print(f'Saving to "{new_bim_or_pvar_file}"...')
    write_bim_or_pvar(variants, new_bim_or_pvar_file)
